...
//		sceneFrameBuffer.color[0] = RenderBuffer(m_window.config.get<Window::Size>(), Channels::RGB, 8);
//		sceneFrameBuffer.depth    = RenderBuffer(m_window.config.get<Window::Size>(), Channels::Depth, 8);
//
//		ResolverLayer    resolveLayer(Channels::RGB);
//		PostProcessLayer postProcessLayer("u_texture", m_assets.getWeak<dk::gfx::ShaderSource>("/shaders/chromatic_aberration_fs.glsl"));
//
//		while (m_window.isOpen()) {
//			const auto& frame = m_window.beginFrame();
//			// Clear backbuffer
...
//
//				auto& resolvedSceneTex = resolveLayer(sceneFrameBuffer);
//				auto& sceneColorTex    = postProcessLayer(resolvedSceneTex);
...
//		m_shaders.insert("chromatic_aberration", std::make_tuple(ShaderSource::postProcessVertexSource(), m_assets.getWeak<ShaderSource>("/shaders/chromatic_aberration_fs.glsl")));
//
//		// Load skybox
