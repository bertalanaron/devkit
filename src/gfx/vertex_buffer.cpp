...
dk::gfx::VertexBuffer dk::gfx::VertexBuffer::create(VertexFlags flags)
{
    ...
    return VertexBuffer(attributes, std::move(dk::common::RuntimeBuffer::create(attributes->size())));
}

dk::gfx::VertexBuffer dk::gfx::VertexBuffer::create(const VertexAttributes* vertexAttributes)
{
    return VertexBuffer(vertexAttributes, std::move(dk::common::RuntimeBuffer::create(vertexAttributes->size())));
}
...
void dk::gfx::VertexBuffer::makeActive()
{
    ...
    if (m_resized) {
        ...
        glBufferData(GL_ARRAY_BUFFER, m_vertexAttributes->size() * size(), m_vertices.data(), GL_STATIC_DRAW);
    }
    else
        update();
}
...
void dk::gfx::VertexBuffer::push(const std::vector<uint8_t>& vertex)
{
